...
class TransformerTandIClassifier(TandIClassifier):
    ...
    def load_model(self):
        ...
        if os.path.exists(f"app/trends_innovations_classifier/checkpoints/{self.model_name}"):
            logging.info("Loading preexisting model...")
            model = transformers_lib[self.model_name].from_pretrained(
                f"app/trends_innovations_classifier/checkpoints/{self.model_name}",
                local_files_only=True
            ).to(self.config["device"])
            tokenizer = AutoTokenizer.from_pretrained(f"app/trends_innovations_classifier/checkpoints/{self.model_name}", local_files_only=True)
    ...
    def predict(self, samples: List[str]):
        ...
        le = load(open(f'app/trends_innovations_classifier/checkpoints/{self.model_name}/label_encoder.pkl', 'rb'))
